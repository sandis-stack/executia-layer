...
create table if not exists governance_freeze_events (
  id uuid primary key default gen_random_uuid(),
  freeze_id uuid not null references governance_freezes(id) on delete cascade,
  organization_id uuid not null,
...
create index if not exists idx_governance_freezes_org on governance_freezes(organization_id);
create index if not exists idx_governance_freezes_review on governance_freezes(review_id);
create index if not exists idx_governance_freezes_execution on governance_freezes(execution_id);
create index if not exists idx_governance_freezes_status on governance_freezes(status);
create index if not exists idx_governance_freeze_events_freeze on governance_freeze_events(freeze_id);
...
check (freeze_level in ('L1','L2','L3','EMERGENCY'));
