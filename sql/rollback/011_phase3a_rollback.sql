-- EXECUTIA™ Phase 3A rollback
-- 1) Restore pre-3A commit_execution and commit_operator_decision (inline hash).
-- 2) Drop Phase 3A ledger authority functions.
-- Run in Supabase SQL editor if Phase 3A must be reverted.
...
DROP FUNCTION IF EXISTS executia_ledger_append(uuid, text, text, jsonb);
DROP FUNCTION IF EXISTS executia_get_last_ledger_hash();
DROP FUNCTION IF EXISTS executia_ledger_entry_hash(uuid, text, text, text);
