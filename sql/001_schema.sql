...
-- ============================================================
-- RLS POLICIES (service_role bypass — required for backend access)
-- ============================================================
-- All backend access uses service_role key which bypasses RLS.
-- These policies explicitly allow service_role full access.
-- For future tenant isolation, add per-organization policies.

CREATE POLICY "service_role_all_execution_results"
  ON execution_results FOR ALL TO service_role USING (true) WITH CHECK (true);
...
CREATE POLICY "service_role_all_core_ledger"
  ON core_ledger FOR ALL TO service_role USING (true) WITH CHECK (true);

CREATE POLICY "service_role_all_ledger_accounts"
  ON ledger_accounts FOR ALL TO service_role USING (true) WITH CHECK (true);

CREATE POLICY "service_role_all_truth_anchors"
  ON truth_anchors FOR ALL TO service_role USING (true) WITH CHECK (true);

CREATE POLICY "service_role_all_organizations"
  ON organizations FOR ALL TO service_role USING (true) WITH CHECK (true);

CREATE POLICY "service_role_all_organization_users"
  ON organization_users FOR ALL TO service_role USING (true) WITH CHECK (true);

-- ============================================================
-- ATOMIC EXECUTION RPC
-- Run sql/009_atomic_execution_rpc.sql separately after this schema.
-- The RPC commit_execution_transaction() handles:
--   execution_results + audit_events + ledger_entries in one transaction
--   with pg_advisory_xact_lock to prevent hash chain race conditions.
-- ============================================================
