...
export default async function handler(req, res) {
  try {
    ...
    if (req.method === "GET") {
      return json(res, 200, {
        ok: true,
        endpoint: "/api/v2/governance/review/reject",
        method: "POST",
        purpose: "Reject governance review escalation."
      });
    }
    ...
    if (!permission.ok) {
      return json(res, permission.status || 401, {
        ...
        error: {
          ...
          message:
            permission.reason ||
            "Governance review rejection permission required."
        }
      });
    }
    ...
    console.error(
      "[EXECUTIA GOVERNANCE REJECT ERROR]",
      error
    );
