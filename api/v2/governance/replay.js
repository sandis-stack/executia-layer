...
export default async function handler(req, res) {
  try {
    ...
    if (!permission.ok) {
      return json(res, permission.status || 401, {
        ...
        error: {
          ...
          message:
            permission.reason ||
            "Governance replay permission required."
        }
      });
    }
    ...
    if (review_id) reviewQuery = reviewQuery.eq("review_id", review_id);
    if (execution_id) reviewQuery = reviewQuery.eq("execution_id", execution_id);
    ...
    if (review_id) freezeQuery = freezeQuery.or(`details->>review_id.eq.${review_id}`);
    if (execution_id) freezeQuery = freezeQuery.or(`details->>execution_id.eq.${execution_id}`);
    ...
    return json(res, 500, {
      ok: false,
      error: {
        code: error.code || "GOVERNANCE_REPLAY_FAILED",
        message:
          error.message ||
          "Governance deterministic replay failed."
      }
    });
  }
}
