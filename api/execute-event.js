...
import { buildExecutionResponse,
         buildErrorResponse }         from "../engine/response-builder.js";
...
export default withEngine(async (req, res) => {
  ...
  if (req.body.contextOverrides && typeof req.body.contextOverrides === "object") {
    try { ctx = mergeEnrichment(ctx, req.body.contextOverrides); }
  ...
  } catch (e) { return err(500, "RULE_FETCH_FAILED", "Cannot load rules — decision aborted.", e.message); }
  ...
  if (invalidRules.length > 0) {
    return err(500, "INVALID_PUBLISHED_RULE",
