...
export default async function handler(req, res) {
  try {
    ...
    if (!authorize(req)) {
      return json(res, 401, {
        ok: false,
        error: { code: "UNAUTHORIZED", message: "Invalid EXECUTIA internal key." }
      });
    }
    ...
    const entry_hash = sha256(ledgerPayload);
    ...
    coreEntry.hash = sha256({ ...coreEntry, prev_hash });
