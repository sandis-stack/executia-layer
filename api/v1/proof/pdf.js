...
export default async function handler(req, res) {
  try {
    ...
    const lines = [
      ...
      `Events: ${proof.autonomous_governance?.events || 0}`,
      `Latest State: ${proof.autonomous_governance?.latest_state || "-"}`,
      `Watchdog: ${proof.autonomous_governance?.latest_watchdog || "-"}`,
      `Orchestrator: ${proof.autonomous_governance?.latest_orchestrator || "-"}`,
      `Containment: ${proof.autonomous_governance?.latest_containment || "-"}`,
      `Recovery: ${proof.autonomous_governance?.latest_recovery || "-"}`,
      "",
      "REPLAY PATH",
