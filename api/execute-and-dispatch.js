...
export default withEngine(
  async (req, res) => {
    ...
    function err(status, code, message, detail = null) {
      return res
        .status(status)
        .json(buildErrorResponse(code, message, detail, requestId));
    }
    ...
    try {
      event = normalizeEventInput({
        ...req.body,
        organizationId:
          req.executia.organizationId || req.body?.organizationId || null,
      });
    } catch (e) {
    ...
    if (
      ...
      typeof req.body.contextOverrides === "object"
    ) {
      try {
    ...
    const { results: evaluatedRules, invalidRules } = evaluateRules(
      scopedRules,
      ctx
    );
    ...
    if (!commitResult.ok) {
      ...
      return res.status(500).json({
        ...buildErrorResponse(
          ...
          commitResult.error_message ||
            "Ledger write failed — decision not committed.",
          null,
          requestId
        ),
        decision: decisionResult.decision,
    ...
    if (decisionResult.decision !== "APPROVE") {
      await logAudit(supabase, {
        ...
        actor_id: req.executia?.auth?.keyId || null,
        actor_label: req.executia?.operatorEmail || null,
        action: "EXECUTION_DECIDED_NO_DISPATCH",
        entity: "ledger",
        entity_id: String(commitResult.ledger_id || ""),
        status: decisionResult.decision === "ESCALATE" ? "review" : "ok",
        request_id: requestId,
        payload: {
          event_type: event.eventType,
          decision: decisionResult.decision,
